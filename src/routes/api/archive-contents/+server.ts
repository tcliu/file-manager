...
export const GET: RequestHandler = async ({ url, request }) => {
  ...
  if (!relativePath) {
    logAccess(request as any, 'archive_view_failed', { path: '', reason: 'missing_file_path' });
  ...
  if (!fileStat?.isFile()) {
    logAccess(request as any, 'archive_view_failed', { path: relativePath, reason: 'file_not_found' });
