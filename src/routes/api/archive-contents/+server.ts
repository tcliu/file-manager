...
export const GET: RequestHandler = async (event) => {
  ...
  if (!relativePath) {
    logAccess(event, 'archive_view_failed', { path: '', reason: 'missing_file_path', elapsed_ms: Date.now() - startedAt });
  ...
  try {
    ...
    logAccess(event, 'archive_view_failed', {
  ...
  if (!fileStat?.isFile()) {
    logAccess(event, 'archive_view_failed', { path: relativePath, reason: 'file_not_found', elapsed_ms: Date.now() - startedAt });
